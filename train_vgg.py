...
parser.add_argument("--hash_bits", type=int, default=12, help="number of hash bits")
...
parser.add_argument("--visual_lr", type=float, default=1e-4, help="initial learning rate for adam")
parser.add_argument("--hash_lr", type=float, default=1e-3, help="initial learning rate for adam")

parser.add_argument("--decay_step", type=int, default=5000, help="number of steps to dacay lreaning rate")
parser.add_argument("--decay_rate", type=float, default=0.5, help="decaying rate")

parser.add_argument("--checkpoint", default=None,
                    help="directory with checkpoint to resume training from or use for testing")
parser.add_argument("--topk", type=int, default=3)
parser.add_argument("--output_dir", default='./models/vgg16-12b-nus-unseen-top3-300d', help="where to put output files")
parser.add_argument("--log", default='./log/vgg16-12b-nus-unseen-top3-300d-log.txt')
parser.add_argument("--save_freq", type=int, default=5000, help="save model every save_freq steps, 0 to disable")
#parser.add_argument("--pretrained_weights", default='weights_resnet.npy',
#                    help="pretrained weight model")
...
if __name__ == '__main__':
    ...
    #model = AlexNet(input_image, dropout_rate, args.hash_bits, args.nb_labels_seen, skip_layer, args.weights_path)
    #hash_layer = model.fc8
   # model = ResNet50(input_image,dropout_rate,args.hash_bits)
    #hash_layer= model.logits
    #visual_fc1 = fc_layer(hash_layer, args.delen, name='visual_fc1', relu=False)
    model = VggNetModel(input_image, args.hash_bits, args.keep_prob, is_training)
    hash_layer = model.score
    visual_fc1 = fc(hash_layer, args.hash_bits, args.delen, name='visual_fc1',relu=False)

    # visual_fc1 = fc(model.fc7, 4096, args.delen, 'visual_fc1', relu=False)
    # hash_layer = fc(visual_fc1, args.delen, args.hash_bits, 'hash_layer', relu=False)
    ...
    visual_rank_loss = ranking_loss(c_matrix1, input_labels) + 2e-4 * l2loss


    # hash_loss = mse_hashing_loss(hash_codes, input_labels, args.hash_bits, alpha)
    hash_loss = hash_focal_loss(hash_layer, input_labels, args.hash_bits, args.alpha)
   
    # List of trainable variables of the layers we want to finetune
    var_list1 = [v for v in tf.trainable_variables() if v.name.split('/')[0] not in skip_layer]
    # List of trainable variables of the layers we want to train
    var_list2 = [v for v in tf.trainable_variables() if v.name.split('/')[0] in skip_layer]

    # learning rate
    hash_lr = tf.train.exponential_decay(args.hash_lr, global_step2, args.decay_step, args.decay_rate, staircase=True)
    ...
    # List of trainable variables of the layers we want to finetune
    visual_var_list1 = [v for v in tf.trainable_variables() if v.name.split('/')[0] not in ['visual_fc1']]
    # List of trainable variables of the layers we want to train
    visual_var_list2 = [v for v in tf.trainable_variables() if v.name.split('/')[0] in ['visual_fc1']]

    # learning rate
    visual_lr = tf.train.exponential_decay(args.visual_lr, global_step1, args.decay_step, args.decay_rate, staircase=True)

    visual_opt1 = tf.train.AdamOptimizer(visual_lr * 0.01)
    ...
    with tf.Session(config=config) as sess:
        ...
        for i_epoch in range(args.num_epochs):
            ...
            # hashcode_unseen_test = sess.run(hash_layer, feed_dict={input_image: test_unseen_x, is_training:False})
            # hashcode_unseen_test = np.sign(hashcode_unseen_test)
